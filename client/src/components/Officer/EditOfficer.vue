<template>
  <div>
    ...
    <form v-on:submit.prevent="editOfficer">
      ...
      <p>
        <button type="submit">แก้ไขข้อมูลเจ้าหน้าที่</button>
        <button v-on:click="navigateTo('/officer')">กลับ</button>
      </p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods:{
        async editOfficer(){
            try{
                await OfficersService.put(this.officer)
                this.$router.push({
                    name: 'officers'
                })
